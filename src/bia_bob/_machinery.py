...
def bob(line: str = None, cell: str = None):
    ...
    if line is not None and line.strip() == "reset":
        Context.chat = []
        return
    ...
    """
    response = ask_llm(task_selection_prompt, model=Context.fast_model)
...
def init_assistant(model=None, auto_execute:bool = False, variables:dict=None, endpoint=None, api_key=None,
                   vision_model=None, keep_history:bool=False, silent:bool=False, system_prompt=None, fast_model=None):
    ...
    if fast_model is None and model is not None:
        fast_model = model # backwardds compatibility
...
def show_disclaimer():
    if Context.disclaimer_shown:
        return
    
    Context.disclaimer_shown = True
    from bia_bob import __version__ as version
    from IPython.display import display, HTML
    from ._utilities import version_string

    if is_notebook():
        remark = version_string(Context.model, Context.vision_model, Context.endpoint, version, Context.fast_model) + ". " + disclaimer(html=True)
        display(HTML(f"""
        ...
        remark = version_string(Context.model, Context.vision_model, Context.endpoint, version, Context.fast_model) + ". " + disclaimer() + colab_disclaimer()
